...
class PetSittersScreen extends StatelessWidget {
  const PetSittersScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Pet Sitters", style: TextStyles.mediumHeading),
      ),
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            .collection('services')
            .where('service',
                isEqualTo: 'pet_sitter') // Filter only pet sitters
            .snapshots(),
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return Center(child: Text("No pet sitters available"));
          }
